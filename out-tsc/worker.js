...
videoQueue.process(async (job, done) => {
    ...
    try {
        ...
        if (acBackgroundJob) {
            ...
            const videoDuration = await encodeVideo(videoInFilename, videoOutFilename, jobData, acBackgroundJob);
...
audioQueue.process(async (job, done) => {
    ...
    try {
        ...
        if (acBackgroundJob) {
            ...
            const audioDuration = await encodeAudio(audioInFilename, audioOutFilename, jobData, acBackgroundJob);
            ...
            console.log('Encode flac');
            await encodeFlac(audioOutFilename, audioOutFlacFilename);
            console.log('Uploading flac');
            await uploadToS3(process.env.S3_AUDIO_PUBLIC_BUCKET, jobData.flacFilename, audioOutFlacFilename);
            acBackgroundJob.progress = 100;
